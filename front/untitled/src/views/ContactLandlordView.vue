...
import { CONTACT_STATUS_OPTIONS, getContactStatusLabel } from '../utils/statusLabels'
...
const statusOptions = CONTACT_STATUS_OPTIONS

async function loadContacts(pageIndex = 0) {
  loading.value = true
  errorMessage.value = ''
  try {
    const response = await contactApi.landlord({ page: pageIndex, size: page.size })
    ...
    errorMessage.value = err.message || '加载咨询记录失败'
  } finally {
    loading.value = false
  }
}

async function updateStatus(recordId, status, remarks) {
  try {
    await contactApi.updateStatus(recordId, { status, remarks })
    successMessage.value = '状态已更新'
    await loadContacts(page.current)
  } catch (err) {
    errorMessage.value = err.message || '更新失败'
  }
}
...
<template>
  <section class="space-y-6">
    <div class="flex items-center justify-between">
      <h2 class="text-base font-semibold text-slate-900">租客咨询</h2>
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="text-xs text-slate-400">租客：{{ contact.tenantName }}</p>
          </div>
        ...
        <div class="mt-3 grid gap-2 text-xs text-slate-500 md:grid-cols-2">
          ...
          <span>备注：{{ contact.remarks || '暂无' }}</span>
        </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-2 text-xs">
          <button
            ...
            :class="contact.status === option.value ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600'"
            @click="updateStatus(contact.id, option.value, contact.remarks || '')"
          >
            设为 {{ option.label }}
          </button>
          <button
    ...
    <HouseChatPanel
      ...
      :header-title="activeContact ? `${activeContact.houseTitle || ''} · ${activeContact.tenantName || ''}` : ''"
      :auto-ensure="false"
